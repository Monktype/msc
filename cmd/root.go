...
func init() {
	...
	predictionCmd.Flags().StringP("channel-name", "c", "", "Target channel name")
	predictionCmd.MarkFlagRequired("channel-name")
	predictionCmd.Flags().StringP("title", "t", "", "Title for prediction")
	predictionCmd.MarkFlagRequired("title")
	predictionCmd.Flags().IntP("window", "d", 0, "Prediction window in seconds (1..1800)")
	predictionCmd.MarkFlagRequired("window")
	predictionResolveCmd.Flags().StringP("channel-name", "c", "", "Target channel name")
	predictionResolveCmd.MarkFlagRequired("channel-name")
	predictionResolveCmd.Flags().StringP("prediction-id", "i", "", "Prediction ID to resolve (printed by the create command)")
	predictionResolveCmd.MarkFlagRequired("prediction-id")
	predictionResolveCmd.Flags().StringP("outcome", "o", "", "Winning outcome: 1 or 2, or the outcome's title")
	predictionResolveCmd.MarkFlagRequired("outcome")
	predictionCmd.AddCommand(predictionResolveCmd)
	predictionCancelCmd.Flags().StringP("channel-name", "c", "", "Target channel name")
	predictionCancelCmd.MarkFlagRequired("channel-name")
	predictionCancelCmd.Flags().StringP("prediction-id", "i", "", "Prediction ID to cancel (printed by the create command)")
	predictionCancelCmd.MarkFlagRequired("prediction-id")
	predictionCmd.AddCommand(predictionCancelCmd)
	predictionLockCmd.Flags().StringP("channel-name", "c", "", "Target channel name")
	predictionLockCmd.MarkFlagRequired("channel-name")
	predictionLockCmd.Flags().StringP("prediction-id", "i", "", "Prediction ID to lock (printed by the create command)")
	predictionLockCmd.MarkFlagRequired("prediction-id")
	predictionCmd.AddCommand(predictionLockCmd)
	...
	rewardsredemptionsCmd.Flags().StringP("after", "a", "", "Pagination cursor to continue from a previous page")
	rewardsredemptionsCmd.Flags().IntP("first", "f", 0, "Number of redemptions to return (max 50)")
	...
	banListCmd.Flags().StringP("channel-name", "c", "", "Target channel name")
	banListCmd.MarkFlagRequired("channel-name")
	banListCmd.Flags().StringP("after", "a", "", "Pagination cursor to continue from a previous page")
	banListCmd.Flags().BoolP("all", "A", false, "Fetch all pages and list every banned/timed-out user (ignores -a)")
	banCmd.AddCommand(banListCmd)
	banAddCmd.Flags().StringP("channel-name", "c", "", "Target channel name")
	banAddCmd.MarkFlagRequired("channel-name")
	banAddCmd.Flags().StringP("user", "u", "", "Target username to ban or time out")
	banAddCmd.Flags().String("user-id", "", "Target user ID to ban/time out (use when the username no longer resolves)")
	banAddCmd.Flags().IntP("duration", "d", 0, "Timeout duration in seconds (0 = perma-ban)")
	banAddCmd.Flags().StringP("reason", "r", "", "Ban reason")
	banCmd.AddCommand(banAddCmd)
	banRemoveCmd.Flags().StringP("channel-name", "c", "", "Target channel name")
	banRemoveCmd.MarkFlagRequired("channel-name")
	banRemoveCmd.Flags().StringP("user", "u", "", "Target username to unban")
	banRemoveCmd.Flags().String("user-id", "", "Target user ID to unban (use when the username no longer resolves; see the [id] shown by ban list)")
	banCmd.AddCommand(banRemoveCmd)
